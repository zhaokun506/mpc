function    [Delta_real,v_real,idx,latError,U ] = ...
    mpc_control(x,y,yaw,refPos_x,refPos_y,refPos_yaw,refKappa,refDelta,dt,L,U,target_v)
%% MPC预设参数
Nx = 3;         % 状态量的个数
Nu = 2;         % 控制量的个数
Np = 15;        % 预测步长
...
%% 原运动学误差状态空间方程的相关矩阵
% 计算参考控制量
%此处1.优化为投影点，2.优化为预瞄点的投影点
idx = calc_target_index(x,y,refPos_x,refPos_y); %距离最近的匹配点
v_r = target_v;%参考速度速度设置为固定速度
Delta_r = refDelta(idx);%
heading_r = refPos_yaw(idx);%参考航向角
kappa_r=refKappa(idx);

% 实际状态量与参考状态量
X_real = [x,y,yaw];%当前状态量和参考状态量

%zhaokun
X_match = [refPos_x(idx), refPos_y(idx)];%匹配点坐标
tor=[cos(heading_r),sin(heading_r)];%匹配点切向量2*1
nor=[-sin(heading_r),cos(heading_r)];%匹配点法向量2*1
d_err=[x-refPos_x(idx),y-refPos_y(idx)];%误差向量1*2
ed=nor*d_err';
es=tor*d_err';
%projection_point_thetar=thetar(dmin);%apollo
X_project=X_match+tor*es;%投影点坐标
projection_point_thetar=heading_r+kappa_r*es;
Xr_project=[X_project(1),X_project(2),projection_point_thetar];
...
% 新的状态量
%zhaokun2022/11/9
%kesi=[x(k),u(k-1)]=[X-Xr,U-Ur]
kesi = zeros(Nx+Nu,1);              % (Nx+Nu) × 1
kesi(1:Nx) = X_real - Xr;
kesi(Nx+1:end) = U;%此处应该为更新的控制量
...
U(1) = kesi(4) + delta_v_tilde; %更新的控制量
